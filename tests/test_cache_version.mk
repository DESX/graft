...
test: | $b
	...
	@test -f $(GRAFT_CACHE)/miniz-2.1.0.tar.gz || (echo "ERROR: default tar not versioned (2.1.0)" && exit 1)
	@echo "  build 2.1.0: OK"
	...
	@test -f $(GRAFT_CACHE)/miniz-3.0.2.tar.gz || (echo "ERROR: default tar not versioned (3.0.2)" && exit 1)
	@echo "  bump to 3.0.2 re-extracted: OK"

	@# ── Both versioned archives coexist (switching back reuses the cache) ──
	@test -f $(GRAFT_CACHE)/miniz-2.1.0.tar.gz && test -f $(GRAFT_CACHE)/miniz-3.0.2.tar.gz \
